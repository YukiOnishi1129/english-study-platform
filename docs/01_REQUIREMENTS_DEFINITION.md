...
- **英→日モード（Recognition）**: 英単語（見出し語）を提示し、日本語訳を入力または選択して回答する。
- **日→英モード（Recall）**: 日本語訳を提示し、英単語のスペルを入力して回答する。
- **英作文モード（Sentence）**: 指定された英単語を必ず含む英文を入力し、登録済みの例文と照合して判定する。
- **モード切替**: 1語彙レコードに英語・日本語・例文を保持し、学習セッション側で3つのタブを切り替えて出題（語彙ごとの個別設定は不要）。通常学習ではユーザーが任意に切り替え、最後に利用したモードをクッキーに保存してデフォルト表示に反映する。復習セッションでは蓄積した統計から推奨モードを自動判定し、そのモードで出題する（手動切り替えは行わない）。
- **表示情報**: 品詞、アクセント、類義語／対義語／関連語を回答後のフィードバックで表示し、英語の例文には音声再生ボタンを用意する。
...
- 管理者は利用者一覧を確認できる（氏名、メールアドレス、ロール、最終ログイン日時、利用可否ステータス）。検索（氏名/メール）、ロール・ステータスでの絞り込み、最終ログイン日時によるソートを行える。
- 利用者詳細では、Google 連携で取得したプロフィール、現在のロール、最終ログイン日時などの基本情報を閲覧できる。自分自身の詳細も閲覧可能だが、ロール/ステータス変更操作は行えない。
- 管理者は一般ユーザー⇔管理者のロール変更ができ、変更履歴（変更日時・変更者）を残す。変更時に対象ユーザーへトースト通知・システムログを記録する。
- アカウントの利用可否を切り替えられる（soft delete）。停止時は対象ユーザーを即時サインアウトさせ、再度ログインしようとすると「利用停止中」で弾く。再開時は通常ログイン可能に戻す。 *** End Patch
...
- 弱点問題の自動抽出・集中学習
- 学習分析ダッシュボード（累計学習時間・正答率推移・教材/UNIT別ハイライト・セッション詳細・復習リコメンドを段階的に追加予定）
- ヘルプセンター（FAQ/検索、トラブルシュート、お問い合わせフォーム、リリースノートの集約）
