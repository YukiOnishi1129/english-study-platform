...
教材タイプ（`content_type`）に応じて利用するテンプレートが異なるため、以下では代表的な 2 タイプ（フレーズ教材 / 語彙教材）の仕様を示します。今後、新しい教材タイプが追加された場合は同様にテンプレートとマッピングを拡張してください。

---

## 1. カラム定義（フレーズ教材 / content_type = `phrase`）
...
| 音声URL       | 任意 | 音声ファイルの URL（`phrase_questions.audio_url`）。                                                                     |
| 英語正解1      | ✅   | 正解文のメイン回答。                                                                                                    |
...
## 1.2 カラム定義（語彙教材 / content_type = `vocabulary`）

語彙教材用テンプレートでは語彙メタ情報と出題設定を同時に取り込みます。

| 列名                 | 必須 | 内容 |
|----------------------|------|------|
| 語彙ID               | 任意 | 既存の語彙エントリ（`vocabulary_entries.id`）を更新する場合に指定。空欄は新規エントリ。 |
| 問題ID               | 任意 | 既存問題を更新する場合に指定（`question_id`）。`関連ID` 列としても読み込み可能。 |
...
| 正解候補1            | ✅   | Questionに登録する正解。`jp_to_en` モードでの回答として利用。 |
...
| 例文(英)             | 任意 | 英語の例文（`vocabulary_questions.example_sentence_en`）。 |
| 例文(和)             | 任意 | 例文の日本語訳（`vocabulary_questions.example_sentence_ja`）。 |
